...
class _RefeicoeScreenState extends State<RefeicoeScreen> {
  ...
  // Refeições com confirmação em andamento — evita duplo-toque (DEF-009).
  ...
  Future<void> _confirmarRefeicao({
    ...
    // Trava de duplo-toque (DEF-009): ignora toques enquanto esta mesma
    // refeição já está sendo processada.
    if (_processando.contains(mealFirestoreName)) return;
    setState(() => _processando.add(mealFirestoreName));
    ...
    try {
      if (jaConfirmada) {
        // ── DESCONFIRMAR ──────────────────────────────────────
        // Tudo dentro de uma transação (DEF-008): a leitura do snapshot e a
        // subtração no doc do usuário acontecem de forma atômica.
        final calRemovida = await FirebaseFirestore.instance
            .runTransaction<double>((tx) async {
          final refSnap  = await tx.get(refeicaoDoc);
          ...
          if (refSnap.exists) {
            final d = refSnap.data() as Map<String, dynamic>;
            // Usa o snapshot gravado; se não existir, usa o valor atual.
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Refeição desmarcada. -${calRemovida.toInt()} kcal removidas.',
              ),
        ...
        // ── CONFIRMAR ─────────────────────────────────────────
        // Os dois writes (snapshot na refeição + incremento no usuário)
        // ocorrem atomicamente dentro de uma transação (DEF-008).
      ...
      // Libera a trava de duplo-toque, esteja a tela montada ou não.
      if (mounted) {
        setState(() => _processando.remove(mealFirestoreName));
      } else {
        _processando.remove(mealFirestoreName);
      }
    }
  }
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      // Desabilitado enquanto esta refeição está em
                      // processamento, reforçando a trava (DEF-009).
                      onPressed: _processando.contains(meal.firestoreName)
                          ? null
                          : () => _confirmarRefeicao(
